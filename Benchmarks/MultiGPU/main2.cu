...
#pragma omp parallel num_threads(deviceCount)
	...
	//	unsigned int myLastIdx = (myFirstIdx + arrayOffset) > probSize ? probSize - 1 : (myFirstIdx + arrayOffset);
		...
		cudaMalloc((void**)&_Dt,sizeof(REAL)*arrayOffset);								cudaMemcpy(_Dt,t + myFirstIdx,sizeof(REAL)*arrayOffset,cudaMemcpyHostToDevice);
		cudaMalloc((void**)&_Dy,sizeof(REAL)*4*arrayOffset);							cudaMemcpy(_Dy,y + myFirstIdx*4,sizeof(REAL)*4*arrayOffset,cudaMemcpyHostToDevice);
		cudaMalloc((void**)&_Djt,sizeof(int)*arrayOffset);								cudaMemcpy(_Djt,jt + myFirstIdx,sizeof(int)*arrayOffset,cudaMemcpyHostToDevice);
		cudaMalloc((void**)&_Dneq,sizeof(int)*arrayOffset);								cudaMemcpy(_Dneq,neq + myFirstIdx,sizeof(int)*arrayOffset,cudaMemcpyHostToDevice);
		cudaMalloc((void**)&_Dliw,sizeof(int)*arrayOffset);								cudaMemcpy(_Dliw,liw + myFirstIdx,sizeof(int)*arrayOffset,cudaMemcpyHostToDevice);
		cudaMalloc((void**)&_Dlrw,sizeof(int)*arrayOffset);								cudaMemcpy(_Dlrw,lrw + myFirstIdx,sizeof(int)*arrayOffset,cudaMemcpyHostToDevice);
		cudaMalloc((void**)&_Datol,sizeof(REAL)*arrayOffset);							cudaMemcpy(_Datol,atol + myFirstIdx*1,sizeof(REAL)*arrayOffset,cudaMemcpyHostToDevice);
		cudaMalloc((void**)&_Ditol,sizeof(int)*arrayOffset);							cudaMemcpy(_Ditol,itol + myFirstIdx,sizeof(int)*arrayOffset,cudaMemcpyHostToDevice);
		cudaMalloc((void**)&_Diopt,sizeof(int)*arrayOffset);							cudaMemcpy(_Diopt,iopt + myFirstIdx,sizeof(int)*arrayOffset,cudaMemcpyHostToDevice);
		cudaMalloc((void**)&_Drtol,sizeof(REAL)*arrayOffset);							cudaMemcpy(_Drtol,rtol + myFirstIdx,sizeof(REAL)*arrayOffset,cudaMemcpyHostToDevice);
		cudaMalloc((void**)&_Diout,sizeof(int)*arrayOffset);							cudaMemcpy(_Diout,iout + myFirstIdx,sizeof(int)*arrayOffset,cudaMemcpyHostToDevice);
		cudaMalloc((void**)&_Dtout,sizeof(REAL)*arrayOffset);							cudaMemcpy(_Dtout,tout + myFirstIdx,sizeof(REAL)*arrayOffset,cudaMemcpyHostToDevice);
		cudaMalloc((void**)&_Ditask,sizeof(int)*arrayOffset);							cudaMemcpy(_Ditask,itask + myFirstIdx,sizeof(int)*arrayOffset,cudaMemcpyHostToDevice);
		cudaMalloc((void**)&_Diwork,sizeof(int)*24*arrayOffset);						cudaMemcpy(_Diwork,iwork + myFirstIdx*24,sizeof(int)*24*arrayOffset,cudaMemcpyHostToDevice);
		cudaMalloc((void**)&_Drwork,sizeof(REAL)*86*arrayOffset);						cudaMemcpy(_Drwork,rwork + myFirstIdx*86,sizeof(REAL)*86*arrayOffset,cudaMemcpyHostToDevice);
		cudaMalloc((void**)&_Distate,sizeof(int)*arrayOffset);							cudaMemcpy(_Distate,istate + myFirstIdx,sizeof(int)*arrayOffset,cudaMemcpyHostToDevice);
		cudaMalloc((void**)&_Dcommon,sizeof(struct cuLsodaCommonBlock)*arrayOffset);	cudaMemcpy(_Dcommon,&Hcommon[myFirstIdx],sizeof(struct cuLsodaCommonBlock)*arrayOffset, cudaMemcpyHostToDevice);
		cudaMalloc((void**)&_Derr,sizeof(int)*arrayOffset);								cudaMemcpy(_Derr,istate + myFirstIdx,sizeof(int)*arrayOffset,cudaMemcpyHostToDevice);
